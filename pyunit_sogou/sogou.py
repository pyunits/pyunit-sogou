...
class SoGou:
    def __init__(self, search):
        ...
        """
        ua = UserAgent()  # 设置UA
        url_word = quote(search.encode('GBK'))  # 将中文字转化为URL链接。注意搜狗将中文字进行的GBK编码。而不是UTF-8
        ...
        for page in range(1, max_page + 1):
            response = requests.get(url=url + str(page), headers={'User-Agent': ua.random})
    ...
    def load_word(self, url_word=None):
        ...
        if not url_word:
            for u in self.load_url:
                ...
                response = requests.get(url=u, headers={'User-Agent': ua.random})
                ls = to_txt(response.content)  # 将其转化为UTF-8格式
                load_ls[name] = ls
        else:
            name = url_word[url_word.rfind('=') + 1:]
            response = requests.get(url=url_word, headers={'User-Agent': ua.random})
...
if __name__ == '__main__':
    sg = SoGou('电子游戏')  # 获取搜索关键字下的词库
    for url in sg.load_url:  # 遍历关键字下的URL
        txt = sg.load_word(url)  # 下载
        print(txt)  # 打印
